...
- [x] **Eliminar la duplicación de formularios de prospecto.** `collectPros()` (alta, `app.html:1789-1800`, HTML en `1767-1785`) y `prosCamposCollect()` (edición, `app.html:1945-1956`, HTML en `1925-1943`) son ~15 campos copy-pasteados. Extraer a una función/generador de HTML compartido.

  > Hecho (10-sep-2026). Se creó un descriptor `PROS_CAMPOS` (16 campos + Notas aparte) y dos funciones compartidas — `prosFormFieldsHTML(prefix, d)` (arma el grid + Notas) y `prosFormCollect(prefix)` (lee el DOM y arma el payload) — ubicadas junto a `sectorSelectHtml`. `prosRegistrar()` (alta, prefijo `px_`) y `prosGestionar()` (edición, prefijo `pg_f_`) las usan en vez de tener 17 campos copy-pasteados cada una; se eliminaron `collectPros()`, `prosCamposHTML()` y `prosCamposCollect()`. El id del `<datalist>` de cargos sugeridos pasó de `dl_cargos`/`pg_dl_cargos` a `px_dl_cargos`/`pg_f_dl_cargos` (sin otras referencias al literal viejo, confirmado por grep). **Decisión de producto**: el campo Sector mostraba el asterisco de obligatorio solo en alta, sin que ni el backend (columna nullable) ni el formulario de edición lo trataran como tal — se quitó el asterisco de los dos formularios en vez de agregar una validación nueva. Verificado en vivo: alta y edición de un prospecto real (guardado, cambio de sector con re-render preservando lo tipeado, eliminación), sin errores de consola nuevos tras corregir un bug propio introducido en el refactor (branch de fecha usaba `v.slice` en vez de `(v||'').slice`, corregido antes de terminar la sesión).

- [x] **Asociar `<label for="">` con su input.** Hoy 0 de 107 `<label>` en `app.html` tienen `for` apuntando al `id` del control (ej. `app.html:1768`: `<label>Sector *</label>` seguido de `<select id="px_sector">` sin vínculo). Rompe accesibilidad y el comportamiento nativo de "click en label enfoca el input".

  > Hecho (10-sep-2026). De los 75 `<label>` que quedaron en el archivo tras el ítem anterior (la grilla compartida de prospecto ya nace con `for` correcto por construcción), 64 recibieron `for="<id-del-control>"`. Los 11 restantes se dejaron sin `for` a propósito: 7 son labels que ya envuelven al control (checkboxes e inputs de fecha en Timbrado/automatizaciones/board-miembros — la asociación nativa ya funciona sin el atributo) y 4 son labels de grupo sobre un conjunto de botones/checkboxes sin un único control al que apuntar (Story points, tier-grid de créditos, "Miembros del proyecto", "Proyectos con sprint semanal"). Verificado en vivo (clic en label enfoca el input correcto) en los formularios de prospecto, "Nuevo usuario" y contraseña.

- [x] **Validar todos los campos marcados como obligatorios.** Hoy `collectPros()` solo valida "empresa" (`app.html:1812-1822`); hay 29 `form-required-marker` (el asterisco visual) pero solo 23 atributos `required` en los `<input>` — desalineado. Agregar validación real (JS o al menos `required` consistente) para todos los campos marcados.

  > Investigado (10-sep-2026): de los 8 campos con asterisco pero sin `required` real, solo el Sector de prospecto era una inconsistencia genuina (resuelta en el ítem de dedup de arriba, quitando el asterisco). El resto **no eran bugs**: el wizard de créditos (`w_project`, `w_just`, tier-grid) y `nu_position` ya tienen validación real vía JS (rechazan el submit si falta el dato), solo no usan el atributo HTML `required` porque son wizards multi-paso o campos condicionales donde `required` nativo no aplicaría bien; y los selects `hr_type`/`nu_type`/`nu_role` siempre traen un valor por defecto — no hay estado vacío posible, así que no hay nada que validar. Decisión de producto confirmada con el usuario: dejar esos 3 selects como están (asterisco cosmético, sin bug real detrás). No se tocó código adicional más allá del ítem de dedup.

- [x] **Crear una función `emptyState(msg)` reutilizable.** Hoy hay 6 mensajes de "vacío" escritos a mano con padding distinto cada uno: `app.html:1140` ("El backlog está vacío..."), `1033` ("No hay actividades..."), `1874` ("Sin prospectos..."), `2488` ("Sin datos."), `2705` ("Sin registros en el rango."), `2765` ("No hay colaboradores.").

  > Hecho (10-sep-2026). Se creó `emptyState(msg, opts)` junto a `chipEstado`/`avatarHtml`: sin `opts` arma un `<p class="text-gray text-center">` centrado, con `opts.colspan` arma un `<tr><td colspan="N">` para reemplazar el `<tbody>` de una tabla vacía. Se migraron los 6 call-sites. Cambio de apariencia menor y aceptado (mismo criterio que la unificación de badges/tablas de la sesión anterior): los 3 mensajes que no tenían padding/centrado (Sin datos., Sin registros en el rango., No hay colaboradores.) lo ganaron, quedando visualmente iguales a los otros 3. Verificado en vivo en la lista de prospectos ("Sin prospectos con estos filtros." centrado y con padding).

- [x] **Usar el `.spinner` ya definido en CSS (`app.css:937-948`) también en el CRM.** Hoy solo se usa en `login.html`/`register.html`/`verify-email.html`; en `app.html` el único estado de carga es el texto "Cargando..." reemplazando todo `#view` (`app.html:265` y variantes en `618,2033,3170`).

  > Hecho (10-sep-2026). Se creó `loadingHtml(msg)` (mismo bloque de helpers) que arma un `<span class="spinner"></span>` centrado, con mensaje opcional al lado. Reemplazó el texto "Cargando..."/"Cargando…" en las 5 apariciones: el router (`#view` antes de despachar la ruta), `#hrList` y `#reqList` (solicitudes de RRHH/créditos) y `#pg_timeline` (historial de prospecto) vía `loadingHtml()`; el HTML estático inicial del shell (antes de que cargue el JS) se escribió a mano con el mismo markup ya que no puede llamar a una función JS. Verificado que `.spinner` computa `20px`/`50%`/animación `spin` como en `login.html`.
...
>
> Quedó explícitamente fuera de esa sesión (Fase 2): la implementación real del modal (se optó por eliminar el CSS muerto en vez de implementarlo — ver nota arriba). Los otros 5 ítems entonces pendientes (deduplicar formularios de prospecto, asociar `<label for="">`, validar campos obligatorios, `emptyState()` reutilizable, uso del `.spinner` en el CRM) se completaron en una sesión posterior — ver las notas "Hecho (10-sep-2026)" en cada ítem de arriba. Con esto, Fase 2 queda completa.
